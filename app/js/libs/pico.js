...
/**
 * Unpacks the bytes of the cascade file.
 *
 * @param bytes
 * @returns {classify_region}
 */
    ...
    function classify_region(r, c, s, pixels, ldim) {
        ...
        for(let i = 0; i < ntrees; ++i) {
            ...
            for(let j = 0; j < tdepth; ++j) {
                ...
                const idx1 = ((r + tcodes[root + 4 * idx + 0] * s) >> 8) * ldim + ((c + tcodes[root + 4 * idx + 1] * s) >> 8);
                const idx2 = ((r + tcodes[root + 4 * idx + 2] * s) >> 8) * ldim + ((c + tcodes[root + 4 * idx + 3] * s) >> 8);

                // RGBA: Use this line if the pixels array is rgba (4 bytes per pixel)
	            ///idx = 2 * idx + (pixels[idx1 * 4] <= pixels[idx2 * 4]);

                // GRAYSCALE: Use this line if the pixels array is grayscale (1 bytes per pixel)
	            idx = 2 * idx + (pixels[idx1] <= pixels[idx2]);
            }
